...
async def Fish_menu():
        ...
        if fish_choosed == 1:
            ...
            while fish_choosed == 0:
                ...
                for i in range(0, len(fish_list)):
                    ...
                    if randint_fish > ((100 / fish_forloop_count) *
                                       (rod_boost)):
                        rand_fish_emoji = ["🐟", "🐠", "🐡", "🦈"]
                        caught_fish = fish_list[i] + rand_fish_emoji[randint(
                            0, 3)]
                        caught_fish_inventory_addon = fish_list[i]
                        fish_credit += i * 100
                        fish_choosed = 1
    ...
    async def fish():
        ...
        try:
            print()
            user_input = await asyncio.wait_for(asyncio.to_thread(input),
                                                time_)
